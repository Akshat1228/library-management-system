...
public class ReturnBook extends JFrame implements ActionListener{
    ...
    public void delete() {
        try {
            conn con = new conn();
            String sql = "delete from issueBook where book_id=? and dateOfIssue=?";
            PreparedStatement st = con.c.prepareStatement(sql);
            st.setString(1, textField.getText());
            st.setString(2, textField_6.getText());
    ...
    public void actionPerformed(ActionEvent ae){
		...
		rs.close();
		  ...
		  int i=0;
                if(textField_3.getText().compareToIgnoreCase("")!=0 && textField_6.getText().compareToIgnoreCase("")!=0){
                PreparedStatement st = con.c.prepareStatement(sql);
		st.setString(1, textField.getText());
